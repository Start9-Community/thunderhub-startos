...
This repo packages [ThunderHub](https://github.com/apotdevin/thunderhub) for StartOS.
...
ThunderHub runs the `apotdevin/thunderhub` image pinned via `dockerTag` in `startos/manifest/index.ts`.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `apotdevin/thunderhub:<new version>`.
